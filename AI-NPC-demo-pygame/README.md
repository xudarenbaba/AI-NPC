...
- **输入限制**：当前 Demo 仅支持**英文键盘输入**（不支持中文输入与粘贴）。
- **焦点**：Pygame 没有独立「输入框控件」，键盘输入只会发给**当前获得焦点的窗口**。从终端启动时焦点常在终端，**请先鼠标单击游戏窗口**再输入；按 E 后会尝试把游戏窗置顶（Windows），若仍无效请手动点一下游戏画面。
